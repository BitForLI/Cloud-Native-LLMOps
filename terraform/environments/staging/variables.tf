...
variable "api_max_capacity" {
  description = "Maximum API tasks allowed by staging autoscaling."
  type        = number
  default     = 6

  validation {
    condition     = var.api_max_capacity >= 2 && floor(var.api_max_capacity) == var.api_max_capacity
    error_message = "Staging API maximum capacity must be an integer of at least two."
  }
}

variable "worker_max_capacity" {
  description = "Maximum Worker tasks allowed by staging autoscaling."
  type        = number
  default     = 10

  validation {
    condition     = var.worker_max_capacity >= 2 && floor(var.worker_max_capacity) == var.worker_max_capacity
    error_message = "Staging Worker maximum capacity must be an integer of at least two."
  }
}

variable "worker_backlog_target_per_task" {
  description = "Visible inference jobs targeted per running staging Worker."
  type        = number
  default     = 2
