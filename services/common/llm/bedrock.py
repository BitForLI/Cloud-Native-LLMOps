...
class BedrockConfig(Protocol):
    ...
    bedrock_input_cost_per_million_tokens: float | None
    bedrock_output_cost_per_million_tokens: float | None
...
class BedrockProvider:
    ...
    def __init__(self, settings: BedrockConfig, client: BaseClient | None = None) -> None:
        self.region = settings.aws_region
        self.model_id = settings.bedrock_model_id
        self.max_tokens = settings.bedrock_max_tokens
        self.temperature = settings.bedrock_temperature
        self.input_cost_per_million_tokens = settings.bedrock_input_cost_per_million_tokens
        self.output_cost_per_million_tokens = settings.bedrock_output_cost_per_million_tokens
    ...
    def _parse_usage(self, payload: dict[str, Any]) -> tuple[int | None, int | None]:
        ...
        for count in (input_tokens, output_tokens):
            if count is not None and (
                ...
                raise BedrockResponseError("Bedrock response contains invalid token counts.")
